...
internal object KotestBehaviorSpecDiscoverer : KotestTestTypeDiscoverer {
    ...
    private fun KtCallExpression.findTests(): Set<DiscoveredTest> {
        val type = when (this.calleeExpression?.firstChild?.text) {
            "then", "Then" -> TestType.TEST
            "Context", "context", "Given", "given", "When", "`when`", "And", "and" -> TestType.CONTAINER
